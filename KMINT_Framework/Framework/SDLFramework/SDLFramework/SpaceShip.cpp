...
void SpaceShip::moveForward()
{
	...
	float** backMatrix = back.getMatrix(); // backside of the plane
	...
	Vector normal = direction1.crossProduct(direction2); // normal to backside in forward direction
	normal.normalize();
	std::vector<float> normalVector = normal.getVector();
	for (int i = 0; i < sides_.size(); i++)
		sides_[i] = sides_[i] * MF::translate3d(normalVector[0], normalVector[1], normalVector[2]); // move points along the normal line
}

void SpaceShip::turnHorizontal(bool left)
{
	Matrix top = sides_[0];
	float** topMatrix = top.getMatrix(); // topside of the plane
	Vector topVector1(3);
	...
	Vector normal = direction1.crossProduct(direction2); // normal to topside
	normal.normalize();
	Vector rotationPoint(3); // create rotation point (middle of upper back line)
	...
	if (left)
		rotation = -rotation;
	for (int i = 0; i < sides_.size(); i++)
		sides_[i] = sides_[i] * MF::rotate3d(rotation, rotationPoint, normal); // rotate point around rotation point 10 degrees
}
...
void SpaceShip::turnVertical(bool up)
{
	Matrix top = sides_[0];
	float** topMatrix = top.getMatrix(); // topside of the plane
	Vector rotationAxis(3); // create rotation axis (upper back line)
	...
	if (!up)
		rotation = -rotation;
	for (int i = 0; i < sides_.size(); i++)
		sides_[i] = sides_[i] * MF::rotate3d(rotation, rotationPoint, rotationAxis); // rotate points around rotation point 10 degrees
}
...
void SpaceShip::roll(bool left)
{
	...
	if (left)
		rotation = -rotation;
	for (int i = 0; i < sides_.size(); i++)
		sides_[i] = sides_[i] * MF::rotate3d(rotation, rotationPoint, normal); // rotate points around rotation point 10 degrees
}
...
Matrix SpaceShip::showSight()
{
	...
	float** topMatrix = top.getMatrix();
	// create sight
	Vector start(3); // starting point of sight line
	start.setItem(0, topMatrix[0][0]);
	start.setItem(1, topMatrix[1][0]);
	start.setItem(2, topMatrix[2][0]);
	Vector end(3); // end point of sight line
